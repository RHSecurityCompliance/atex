...
def test_one_remote(image_id):
    with SystemdPodmanProvisioner(image_id) as p:
        shared.one_remote(p)
...
def test_sharing_remote_slot(image_id):
    with SystemdPodmanProvisioner(image_id, max_remotes=1) as p:
...
def test_build_from():
    ...
    with SystemdPodmanProvisioner.build_from(pulled) as p:
        p.provision(1)
        remote = p.get_remote()
        remote.cmd(("systemctl", "status", "1"), check=True)
        remote.cmd(("systemctl", "list-units"))
        remote.cmd((
            "journalctl", "-b", "--no-pager",
            "_TRANSPORT=stdout", "_TRANSPORT=journal", "_TRANSPORT=syslog",
        ))
