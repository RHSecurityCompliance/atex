...
def test_subtests_grouped(tmp_dir):
...
def test_empty_fields_omitted(tmp_dir):
    """None/empty status, note, subtests, files are omitted entirely."""
    target = tmp_dir / "target.yaml"
    files = tmp_dir / "files"
    artifacts = shared.make_artifacts(tmp_dir, [{}])
    with YAMLDocumentAggregator(target, files) as aggregator:
        aggregator.ingest("platform1", "/test1", artifacts)
    content = target.read_text()
    docs = list(yaml.safe_load_all(content))
...
def test_multiple_results_merged(tmp_dir):
...
def test_subtest_with_files(tmp_dir):
